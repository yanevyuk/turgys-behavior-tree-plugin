...
var BehaviorTree = preload("res://addons/behavior-tree-plugin/BehaviorTree/BehaviorTree.gd");
var BTico =  preload("res://addons/behavior-tree-plugin/BehaviorTree/icon_tree.svg");
var dock
	...
	if amountofbt == 1 and selection.size() == 1:
		...
		if current_dock == null:
			if bt.saveddock == null:
				...
				var newdock = preload("res://addons/behavior-tree-plugin/GraphEditor/VisualEditor.tscn").instance(PackedScene.GEN_EDIT_STATE_INSTANCE)
