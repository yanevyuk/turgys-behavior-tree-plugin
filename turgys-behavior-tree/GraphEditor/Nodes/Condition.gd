...
var filedialog
export(String) var conditionname = "CONDITION" setget set_name

func set_name(val):
	conditionname = val.to_upper()
	if has_node("Name"):
		$Name.text = conditionname
...
func set_actionscript(val: GDScript):
	if val != null:
		...
		actionscript = val;
		$TextureButton.texture_normal = load("res://addons/behavior-tree-plugin/GraphEditor/Nodes/icon_script.svg")
	else:
		$TextureButton/ScriptName.text = ""
		actionscript = val;
		$TextureButton.texture_normal = load("res://addons/behavior-tree-plugin/GraphEditor/Nodes/icon_script_create.svg")
	...
	if actionscript!=null:
		if val != delete_mode:
			if val == false:
				...
				$TextureButton.texture_normal = load("res://addons/behavior-tree-plugin/GraphEditor/Nodes/icon_script_remove.svg")
	else:
		$TextureButton.texture_normal = load("res://addons/behavior-tree-plugin/GraphEditor/Nodes/icon_script_create.svg")

var template = load("D:\\Godot\\Projects\\Behavior Tree Plugin\\addons\\behavior-tree-plugin\\BehaviorTree\\Templates\\action.gd")
func _ready():
	if actionscript == null:
		$TextureButton.texture_normal = load("res://addons/behavior-tree-plugin/GraphEditor/Nodes/icon_script_create.svg")
	else:
	...
	filedialog = $TextureButton/FileDialog;
	pass
